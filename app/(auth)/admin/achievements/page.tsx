...
const EMPTY_FORM: Partial<Achievement> = {
  name: '', icon: '🏆', category: 'explorer', description: '', triggerCondition: '', isActive: true,
};

const CATEGORIES: { value: Achievement['category']; label: string; color: string }[] = [
  { value: 'explorer',  label: 'Explorer',  color: 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400' },
  ...
  { value: 'community', label: 'Komunitas', color: 'bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400' },
];
...
export default function AdminAchievementsPage() {
  ...
  useEffect(() => {
    ...
    const unsubUA = onSnapshot(
      query(collection(db, 'userAchievements')),
      (snap) => {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (!formData.name?.trim()) { toast.error('Nama achievement wajib diisi'); return; }
    if (!formData.icon?.trim()) { toast.error('Icon emoji wajib diisi'); return; }
    if (!formData.description?.trim()) { toast.error('Deskripsi wajib diisi'); return; }
    if (!formData.triggerCondition?.trim()) { toast.error('Trigger condition wajib diisi'); return; }

    setSaving(true);
    try {
      const id = isEditing
        ? currentId
        : (formData.name!.trim().toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '') + `-${Date.now()}`);
  ...
  const filtered = achievements.filter(a => {
    const qMatch = a.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
        ...
        <table className="w-full text-left text-sm whitespace-nowrap">
          ...
          <tbody className="divide-y divide-stone-100 dark:divide-stone-800">
            {loading || seeding ? (
              ...
              <tr key={ach.id} className={`hover:bg-stone-50 dark:hover:bg-stone-800/50 transition-colors ${!ach.isActive ? 'opacity-50' : ''}`}>
                <td className="p-4 text-2xl text-center">{ach.icon}</td>
                <td className="p-4">
                  <p className="font-bold text-stone-900 dark:text-stone-100">{ach.name}</p>
                  <p className="text-xs text-stone-500 dark:text-stone-400 mt-0.5 whitespace-normal max-w-[220px]">{ach.description}</p>
                </td>
                <td className="p-4"><CategoryBadge category={ach.category} /></td>
                ...
                <td className="p-4 text-center">
                  <span className="font-semibold text-stone-900 dark:text-stone-100">{userCounts[ach.id] || 0}</span>
            ...
            <div className="flex-1 overflow-y-auto p-6">
              <form id="achievement-form" onSubmit={handleSave} className="space-y-5">
                ...
                <div className="space-y-1.5">
                  <label className="block text-xs font-semibold text-stone-700 dark:text-stone-300">Kategori</label>
                ...
                <div className="space-y-1.5">
                  ...
                  <p className="text-[10px] text-stone-400 leading-relaxed">
                    ID unik yang digunakan sistem untuk mencocokkan kondisi pencapaian. Gunakan format kebab-case.
                  </p>
                </div>
                ...
                <div className="flex items-center justify-between py-2 border-t border-stone-100 dark:border-stone-800">
                  <div>
                    <span className="text-sm font-semibold text-stone-900 dark:text-stone-100">Status Badge</span>
                    <p className="text-xs text-stone-400 mt-0.5">
                      {formData.isActive ? 'Badge aktif & bisa diraih user' : 'Badge nonaktif, tersembunyi dari user'}
                    </p>
                  </div>
                  <label className="relative inline-flex items-center cursor-pointer">
                    <input
                ...
                {(formData.name || formData.icon) && (
                  <div className="border border-stone-200 dark:border-stone-700 rounded-xl p-4 bg-stone-50 dark:bg-stone-800/50">
                    <p className="text-xs text-stone-400 mb-2 font-medium uppercase tracking-wide">Preview</p>
                    <div className="flex items-center gap-3">
                      <span className="text-3xl">{formData.icon || '🏆'}</span>
                      <div>
                        <p className="font-bold text-stone-900 dark:text-stone-100 text-sm">{formData.name || 'Nama Achievement'}</p>
                        <p className="text-xs text-stone-500">{formData.description || 'Deskripsi achievement...'}</p>
          ...
          <div className="fixed inset-0 bg-black/50 z-[60] flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in">
            <div className="bg-white dark:bg-stone-900 rounded-2xl max-w-sm w-full p-6 shadow-2xl animate-in zoom-in-95 space-y-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-bold text-stone-900 dark:text-stone-100">Hapus Achievement?</h3>
                  <p className="text-xs text-stone-500 dark:text-stone-400">{target?.icon} {target?.name}</p>
                </div>
              </div>
              ...
              {earnedCount > 0 && (
                <div className="bg-amber-50 dark:bg-amber-900/10 border border-amber-200 dark:border-amber-800/50 rounded-xl p-3">
                  <p className="text-xs text-amber-800 dark:text-amber-300 font-medium">
                    ⚠️ <strong>{earnedCount} user</strong> sudah mendapatkan badge ini. Data riwayat mereka tidak ikut terhapus, hanya metadata achievement ini yang dihapus.
                  </p>
                </div>
              )}

              <p className="text-stone-500 dark:text-stone-400 text-sm">
                Tindakan ini tidak dapat dibatalkan. Achievement akan dihapus permanen dari database.
              </p>
