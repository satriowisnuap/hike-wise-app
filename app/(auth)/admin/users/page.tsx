...
export default function AdminUsersPage() {
  ...
  const filtered = users.filter(u => {
    const qMatch = (u.name?.toLowerCase() || '').includes(searchQuery.toLowerCase()) || 
      ...
      <div className="bg-white dark:bg-stone-900 rounded-2xl border border-stone-200 dark:border-stone-700 overflow-hidden shadow-sm overflow-x-auto">
        <table className="w-full text-left text-sm whitespace-nowrap">
          ...
          <tbody className="divide-y divide-stone-100 dark:divide-stone-800">
              ...
              <tr key={u.uid} className="hover:bg-stone-50 dark:hover:bg-stone-800/50 transition-colors">
                <td className="p-4">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="font-bold text-stone-900 dark:text-stone-100">{u.name || 'Tanpa Nama'}</span>
                  </div>
                </td>
                <td className="p-4 text-stone-600 dark:text-stone-300">{u.email}</td>
                <td className="p-4">
                  {u.role === 'admin' ? (
                    <span className="inline-flex items-center gap-1 px-2 py-1 rounded bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400 text-xs font-semibold"><ShieldCheck className="w-3 h-3"/> Admin</span>
                  ) : (
                    <span className="inline-flex items-center gap-1 px-2 py-1 rounded bg-stone-100 text-stone-700 dark:bg-stone-800 dark:text-stone-400 text-xs font-semibold">User</span>
                  )}
                </td>
                <td className="p-4 font-medium text-stone-900 dark:text-stone-100">{u.totalTrips || 0}</td>
                <td className="p-4 font-medium text-emerald-600 dark:text-emerald-400">{u.ecoScore || 0} pts</td>
            ...
            {paginated.length === 0 && (
              <tr>
                <td colSpan={7} className="p-8 text-center text-stone-500 dark:text-stone-400">Tidak ada data pengguna.</td>
              </tr>
            )}
          </tbody>
        </table>
      ...
      {deleteConfirmId && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in">
          <div className="bg-white dark:bg-stone-900 rounded-2xl max-w-sm w-full p-6 shadow-2xl animate-in zoom-in-95">
            ...
            <p className="text-stone-500 dark:text-stone-400 text-sm mb-6">User ini akan dihapus dari Firestore. Semua trip dan log miliknya bisa menjadi yatim piatu. Lanjutkan?</p>
            <div className="flex gap-3">
              ...
              <button onClick={() => handleDeleteUser(deleteConfirmId)} className="flex-1 py-2 rounded-lg bg-red-600 text-white hover:bg-red-700">
                Ya, Hapus
              </button>
            </div>
          </div>
