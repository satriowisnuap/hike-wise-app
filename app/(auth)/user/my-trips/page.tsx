...
export default function MyTripsPage() {
    ...
    return (
        <div className="max-w-5xl mx-auto space-y-8 pb-20 md:pb-8 flex relative h-full">
            {/* Main List Column */}
            <div
                className={`w-full transition-all duration-300 ${selectedTrip ? 'hidden md:block md:w-1/2 lg:w-5/12 pr-0 md:pr-4' : 'w-full'}`}
            >
                    ...
                    {loading ? (
                        <div className="space-y-4">
                            {[1, 2, 3].map((i) => (
                                <div
                                    key={i}
                                    className="h-32 rounded-2xl bg-stone-100 dark:bg-stone-800 animate-pulse"
                                ></div>
                            ))}
                        </div>
                                    ...
                                    <div className="flex-1 min-w-0 flex flex-col justify-between">
                                        ...
                                        <div className="flex items-center gap-3 text-xs text-stone-600 dark:text-stone-300">
                                            <span className="flex items-center gap-1">
                                                <Clock className="w-3 h-3" /> {trip.duration} Hari
                                            </span>
                                            <span className="flex items-center gap-1">
                                                <Users className="w-3 h-3" /> {trip.members} Org
                                            </span>
                                        </div>
                                    </div>
                                </button>
                        ...
                        <div className="text-center py-16 bg-white dark:bg-stone-900 rounded-2xl border border-stone-200 dark:border-stone-700">
                            ...
                            <p className="text-stone-500 dark:text-stone-400 mb-6 text-sm max-w-xs mx-auto">
                                Anda belum memiliki rencana pendakian yang sesuai dengan filter ini.
                            </p>
                            <Link
                                href="/user/planner"
                                className="inline-flex items-center px-6 py-3 rounded-xl bg-emerald-600 dark:bg-emerald-500 text-white font-medium hover:bg-emerald-700 dark:hover:bg-emerald-600 transition-colors"
                            >
            ...
            {selectedTrip && (
                <div
                    className={`fixed inset-0 z-50 bg-white dark:bg-stone-950 md:static md:bg-transparent md:z-0 transition-all duration-300 md:block md:w-1/2 lg:w-7/12 flex flex-col ${selectedTrip ? 'translate-x-0' : 'translate-x-full md:translate-x-0'}`}
                >
                        ...
                        <div className="p-4 md:p-6 border-b border-stone-200 dark:border-stone-700 bg-stone-50 dark:bg-stone-900/50 flex items-start justify-between shrink-0 pt-10 md:pt-6">
                            <div>
                                <h2 className="text-xl md:text-2xl font-bold text-stone-900 dark:text-stone-100">
                                    {selectedTrip.title
                                        ? `${selectedTrip.title} (${selectedTrip.mountainName})`
                                        : selectedTrip.mountainName}
                                </h2>
                                <p className="text-sm text-stone-500 dark:text-stone-400">
                                    {formatDate(selectedTrip.startDate)} • {selectedTrip.duration} Hari
                                </p>
                                <div className="mt-3 flex gap-2 flex-wrap">
                                    {getStatusBadge(selectedTrip.status)}
                                    {selectedTrip.status === 'planned' && (
                                        <span
                                            className={`px-2 py-1 rounded-full text-xs font-semibold ${isPackingReady ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400' : 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400'}`}
                                        >
                            ...
                            <button
                                ...
                                className="p-2 md:hidden rounded-full hover:bg-stone-200 dark:hover:bg-stone-700 text-stone-500"
                            >
                                <X className="w-6 h-6" />
                            </button>
                        </div>
                            ...
                            {activeTab === 'itinerary' && (
                                <div className="space-y-6">
                                    {selectedTrip.itinerary.map((day, idx) => (
                                        ...
                                        >
                                            <h4 className="font-bold text-stone-900 dark:text-stone-100 mb-3 pb-2 border-b border-stone-100 dark:border-stone-700">
                                                Hari {day.day} - {day.title}
                                            </h4>
                                            <div className="space-y-4">
                                                {day.activities.map((act, i) => (
                                                    <div key={i} className="flex gap-3 text-sm">
                                                        <span className="font-medium text-emerald-600 dark:text-emerald-400 w-12 shrink-0">
                                                            {act.time}
                                                        </span>
                                                        <div>
                                    ...
                                    {!selectedTrip.packingList || selectedTrip.packingList.length === 0 ? (
                                        <div className="text-center py-12">
                                            ...
                                            <Link
                                                ...
                                                className="inline-flex items-center px-4 py-2 rounded-lg bg-emerald-600 dark:bg-emerald-500 text-white text-sm font-medium hover:bg-emerald-700"
                                            >
                                                Buat Packing List
                                            </Link>
                                        </div>
                                            ...
                                            <div className="bg-white dark:bg-stone-800 p-4 rounded-xl border border-stone-200 dark:border-stone-700 mb-6">
                                                <div className="flex justify-between text-sm font-medium mb-2">
                                                    ...
                                                    <span className="text-emerald-600 dark:text-emerald-400">
                                                        {selectedTrip.packingList.reduce(
                                                            (acc, cat) =>
                                                                acc + cat.items.filter((i) => i.checked).length,
                                                            0,
                                                        )}
                                                        /
                                                        {selectedTrip.packingList.reduce(
                                                ...
                                                <div className="h-2 w-full bg-stone-200 dark:bg-stone-700 rounded-full overflow-hidden">
                                                    <div
                                                        ...
                                                        style={{
                                                            width: `${(selectedTrip.packingList.reduce((acc, cat) => acc + cat.items.filter((i) => i.checked).length, 0) / selectedTrip.packingList.reduce((acc, cat) => acc + cat.items.length, 0)) * 100}%`,
                                                        }}
                                                    />
                                                </div>
                                            </div>
                                                ...
                                                >
                                                    ...
                                                    <div className="p-2">
                                                        {cat.items.map((item, iIdx) => (
                                                            <label
                                                                key={item.id}
                                                                className="flex items-center justify-between p-2 hover:bg-stone-50 dark:hover:bg-stone-700/30 rounded-lg cursor-pointer"
                                                            >
                                                                <div className="flex items-center gap-3">
                                                                    ...
                                                                    />
                                                                    <span
                                                                        className={`text-sm ${item.checked ? 'text-stone-400 line-through' : 'text-stone-700 dark:text-stone-300'}`}
                                                                    >
                                                                        {item.name}
                                                                    </span>
                                                                </div>
                                ...
                                <div className="space-y-6">
                                    {selectedTrip.ecoLog?.wasteKg > 0 ? (
                                        <div className="bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800/50 rounded-2xl p-6 text-center">
                                            ...
                                            <h3 className="font-bold text-emerald-900 dark:text-emerald-100 text-lg mb-1">
                                                Eco Log Tersimpan
                                            </h3>
                                            <p className="text-emerald-700 dark:text-emerald-400 text-sm mb-4">
                                                Anda telah mengumpulkan {selectedTrip.ecoLog.wasteKg} kg sampah di
                                                pendakian ini.
                                            </p>
                                        ...
                                        <div className="bg-white dark:bg-stone-800 rounded-2xl border border-stone-200 dark:border-stone-700 p-6 space-y-4">
                                            <div className="text-center mb-6">
                                                ...
                                                <p className="text-sm text-stone-500 dark:text-stone-400">
                                                    Berkontribusi menjaga alam. Berapa kilogram sampah yang Anda bawa
                                                    turun?
                                                </p>
                                            </div>
