...
export default function PackingListPage() {
    ...
    const essentialItems = packingList
        ? packingList.reduce((acc, cat) => acc + cat.items.filter((i) => i.isEssential).length, 0)
        : 0;
    const checkedItems = packingList
        ? packingList.reduce((acc, cat) => acc + cat.items.filter((i) => i.checked).length, 0)
        : 0;
        ...
        <div className="max-w-4xl mx-auto space-y-8 pb-20 md:pb-8">
            ...
            <div className="space-y-2 print:hidden">
                ...
                <p className="text-stone-500 dark:text-stone-400">
                    Buat daftar perlengkapan yang presisi sesuai dengan rencana pendakian Anda.
                </p>
            </div>
                ...
                <div className="lg:col-span-4 space-y-6 print:hidden">
                    <div className="bg-white dark:bg-stone-900 rounded-2xl border border-stone-200 dark:border-stone-700 p-6 space-y-6 shadow-sm">
                        ...
                        <div className="space-y-3">
                            <Select
                                label="Pilih Trip Terencana (Opsional)"
                                value={selectedTripId}
                                onChange={(val) => {
                                    const newId = val as string;
                                    setSelectedTripId(newId);
                                    const trip = plannedTrips.find((t) => t.id === newId);
                                    if (trip) {
                                ...
                                options={[
                                    { label: '-- Tanpa Trip (Generator Mandiri) --', value: '' },
                                    ...plannedTrips.map((t) => ({
                                        label: t.title ? `${t.title} (${t.mountainName})` : t.mountainName,
                                        value: t.id,
                                    })),
                                ]}
                        ...
                        <div className="space-y-2">
                            ...
                            <input
                                ...
                                className="w-full px-4 py-3 rounded-xl border border-stone-300 dark:border-stone-600 bg-white dark:bg-stone-800 text-stone-900 dark:text-stone-100 focus:ring-2 focus:ring-emerald-500 dark:focus:ring-emerald-400 outline-none disabled:opacity-60 disabled:cursor-not-allowed disabled:bg-stone-100 dark:disabled:bg-stone-800"
                            />
                        </div>
                    ...
                    {packingList ? (
                                ...
                                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
                                    <div>
                                        ...
                                        <p className="text-sm text-stone-500 dark:text-stone-400 mt-1">
                                            Total {totalItems} item ({essentialItems} item wajib)
                                        </p>
                                    </div>
                                ...
                                <div className="space-y-2">
                                    <div className="flex justify-between text-sm font-medium">
                                        ...
                                        <span className="text-emerald-600 dark:text-emerald-400">
                                            {checkedItems}/{totalItems} item siap
                                        </span>
                                    </div>
                                ...
                                <div className="space-y-4">
                                    {packingList.map((category, catIdx) => (
                                        <details
                                            ...
                                            className="group rounded-xl border border-stone-200 dark:border-stone-700 bg-white dark:bg-stone-800 overflow-hidden print:border-none print:break-inside-avoid"
                                            open
                                        >
                                                ...
                                                {category.items.map((item, itemIdx) => (
                                                    ...
                                                    >
                                                        <label className="flex items-center gap-3 flex-1 cursor-pointer">
                                                            ...
                                                            <span
                                                                className={`text-sm md:text-base font-medium transition-colors ${item.checked ? 'text-stone-400 dark:text-stone-500 line-through' : 'text-stone-900 dark:text-stone-100'}`}
                                                            >
                                                                {item.name}
                                                            </span>
                                                        </label>
                        ...
                        <div className="h-full min-h-[400px] flex flex-col items-center justify-center bg-stone-50 dark:bg-stone-900/50 rounded-2xl border border-dashed border-stone-300 dark:border-stone-700 print:hidden text-center p-8">
                            ...
                            <p className="text-stone-500 dark:text-stone-400 max-w-sm">
                                Isi detail perjalanan Anda di form sebelah kiri dan klik tombol Buat Packing List untuk
                                melihat hasil.
                            </p>
                        </div>
                    )}
