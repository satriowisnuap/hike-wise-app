...
import { MessageSquare, Filter, Plus, CheckCircle, AlertTriangle, XCircle, Cloud, MapPin, ChevronDown, ChevronUp, X, Clock, Info } from 'lucide-react';
...
export default function CommunityPage() {
  ...
  useEffect(() => {
    ...
    const qApproved = query(collection(db, 'reports'), where('status', '==', 'approved'), orderBy('createdAt', 'desc'));
  ...
  const ConditionBadge = ({ cond }: { cond: 'good' | 'caution' | 'danger' }) => {
    if (cond === 'good') return <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-800/50"><CheckCircle className="w-3 h-3"/> Aman</span>;
    if (cond === 'caution') return <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400 border border-amber-200 dark:border-amber-800/50"><AlertTriangle className="w-3 h-3"/> Waspada</span>;
    return <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400 border border-red-200 dark:border-red-800/50"><XCircle className="w-3 h-3"/> Berbahaya</span>;
  };

  const StatusBadge = ({ status }: { status: string }) => {
    if (status === 'pending') return <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-stone-100 text-stone-600 dark:bg-stone-800 dark:text-stone-400 border border-stone-200 dark:border-stone-700">Menunggu Review</span>;
    if (status === 'approved') return <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-800/50">Disetujui</span>;
    if (status === 'rejected') return <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400 border border-red-200 dark:border-red-800/50">Ditolak</span>;
    return null;
  };
    ...
    <div className="max-w-4xl mx-auto space-y-8 pb-20 md:pb-8">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold text-stone-900 dark:text-stone-100 flex items-center gap-2">
            <MessageSquare className="w-6 h-6 text-emerald-600 dark:text-emerald-500" /> Laporan Jalur Komunitas
          </h1>
      ...
      <div className="flex flex-wrap gap-3 bg-white dark:bg-stone-900 p-4 rounded-2xl border border-stone-200 dark:border-stone-700 shadow-sm">
        ...
        <select 
          ...
          className="px-3 py-1.5 rounded-lg border border-stone-300 dark:border-stone-600 bg-stone-50 dark:bg-stone-800 text-sm font-medium text-stone-900 dark:text-stone-100 outline-none"
        >
      ...
      <div className="space-y-4">
        {visibleReports.map(report => (
          <ReportCard key={report.id} report={report} getRelativeTime={getRelativeTime} ConditionBadge={ConditionBadge} />
        ))}
        ...
        <div className="pt-8 border-t border-stone-200 dark:border-stone-700 space-y-4">
          ...
          <div className="space-y-3">
            {myReports.map(report => (
              <div key={report.id} className="p-4 rounded-xl border border-stone-200 dark:border-stone-700 bg-white dark:bg-stone-900 flex flex-col md:flex-row justify-between gap-4">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    <span className="font-bold text-stone-900 dark:text-stone-100">{report.mountainName}</span>
                    <StatusBadge status={report.status} />
                  </div>
                  <p className="text-sm text-stone-500 dark:text-stone-400 line-clamp-1">{report.description}</p>
          ...
          <div className="bg-white dark:bg-stone-900 w-full max-w-lg rounded-2xl border border-stone-200 dark:border-stone-700 shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-5">
              <div className="space-y-2">
                <label className="block text-sm font-semibold text-stone-900 dark:text-stone-100">Gunung</label>
                <select 
                  required
                  value={formMountainId} 
                  onChange={e => setFormMountainId(e.target.value)}
              ...
              <div className="space-y-2">
                <label className="block text-sm font-semibold text-stone-900 dark:text-stone-100">Kondisi Saat Ini</label>
                <div className="grid grid-cols-3 gap-2">
                  <button type="button" onClick={() => setFormCondition('good')} className={`p-3 rounded-xl border flex flex-col items-center gap-1 ${formCondition === 'good' ? 'bg-emerald-50 dark:bg-emerald-900/30 border-emerald-500 text-emerald-700 dark:text-emerald-400 ring-1 ring-emerald-500' : 'border-stone-200 dark:border-stone-700 text-stone-500 dark:text-stone-400'}`}>
                    <CheckCircle className="w-6 h-6" /> <span className="text-xs font-bold">Aman</span>
                  </button>
                  <button type="button" onClick={() => setFormCondition('caution')} className={`p-3 rounded-xl border flex flex-col items-center gap-1 ${formCondition === 'caution' ? 'bg-amber-50 dark:bg-amber-900/30 border-amber-500 text-amber-700 dark:text-amber-400 ring-1 ring-amber-500' : 'border-stone-200 dark:border-stone-700 text-stone-500 dark:text-stone-400'}`}>
                    <AlertTriangle className="w-6 h-6" /> <span className="text-xs font-bold">Waspada</span>
                  </button>
                  <button type="button" onClick={() => setFormCondition('danger')} className={`p-3 rounded-xl border flex flex-col items-center gap-1 ${formCondition === 'danger' ? 'bg-red-50 dark:bg-red-900/30 border-red-500 text-red-700 dark:text-red-400 ring-1 ring-red-500' : 'border-stone-200 dark:border-stone-700 text-stone-500 dark:text-stone-400'}`}>
                    <XCircle className="w-6 h-6" /> <span className="text-xs font-bold">Berbahaya</span>
                  </button>
                </div>
              </div>

              <div className="space-y-2">
                <label className="block text-sm font-semibold text-stone-900 dark:text-stone-100">Cuaca (Maks 100 Karakter)</label>
                <input 
                  type="text" required maxLength={100}
                  value={formWeather} onChange={e => setFormWeather(e.target.value)}
              ...
              <div className="space-y-2">
                <label className="block text-sm font-semibold text-stone-900 dark:text-stone-100">Deskripsi (Min 50 Karakter)</label>
                <textarea 
                  required minLength={50} maxLength={500} rows={4}
                ...
                <div className="flex justify-between text-xs font-medium text-stone-500 dark:text-stone-400">
                  <span className={formDesc.length > 0 && formDesc.length < 50 ? 'text-red-500' : ''}>
                    {formDesc.length < 50 ? `Kurang ${50 - formDesc.length} karakter` : 'Panjang deskripsi OK'}
                  </span>
              ...
              <div className="bg-blue-50 dark:bg-blue-900/10 p-3 rounded-xl flex gap-3 items-start border border-blue-100 dark:border-blue-900/30">
                ...
                <p className="text-xs text-blue-800 dark:text-blue-300 font-medium">Laporan ditinjau admin sebelum ditampilkan ke publik untuk memastikan validitas informasi.</p>
              </div>
...
function ReportCard({ report, getRelativeTime, ConditionBadge }: { report: TrailReport, getRelativeTime: any, ConditionBadge: any }) {
      ...
      <div className="flex flex-col sm:flex-row justify-between sm:items-start gap-4 mb-4">
        ...
        <div className="flex items-center gap-3 bg-stone-50 dark:bg-stone-800/50 p-2 rounded-xl border border-stone-100 dark:border-stone-800 shrink-0 self-start sm:self-auto">
          ...
          <div>
            ...
            <p className="text-[10px] text-stone-500 flex items-center gap-1"><Clock className="w-3 h-3"/> {getRelativeTime(report.createdAt)}</p>
          </div>
        </div>
      </div>
      
      <div className="relative">
        <p className={`text-sm text-stone-700 dark:text-stone-300 leading-relaxed ${!expanded ? 'line-clamp-2' : ''}`}>
          {report.description}
        </p>
        <button 
        ...
        >
          {expanded ? <>Tutup <ChevronUp className="w-3 h-3"/></> : <>Baca Selengkapnya <ChevronDown className="w-3 h-3"/></>}
        </button>
      </div>
    </div>
  );
